...
def read_data(path):
    ...
    for candidate in range(12):
        ...
        for i in range(number_of_classes * 4):
            data_read_from_file = np.fromfile(os.path.join(path, 'Male' + str(candidate), 'training0', 'classe_%d.dat' % i), dtype=np.int16)
    ...
    for candidate in range(7):
        ...
        for i in range(number_of_classes * 4):
            i=0
            data_read_from_file = np.fromfile(os.path.join(path, 'Female' + str(candidate), 'training0', 'classe_%d.dat' % i), dtype=np.int16)
            data_read_from_file = np.array(data_read_from_file, dtype=np.float32)
            dataset_example = format_data_to_train(data_read_from_file)
            examples.append(dataset_example)
            labels.append((i % number_of_classes) + np.zeros(dataset_example.shape[0]))
